...
excel_scheduler = BackgroundScheduler(
...
def local_excel_job():
    ...
    try:
        logger.info(f"[{datetime.utcnow()}] Checking local Excel file...")
        ...
        logger.exception("Excel job failed")
    finally:
        db.close()

def start_excel_scheduler():
    if not excel_scheduler.running:
        excel_scheduler.add_job(
            local_excel_job,
            IntervalTrigger(seconds=settings.EXCEL_POLL_INTERVAL),
            id="local_excel_ingest",
            replace_existing=True
        )
        excel_scheduler.start()
        logger.info("Excel scheduler started")

def stop_excel_scheduler():
    if excel_scheduler.running:
        excel_scheduler.shutdown()
