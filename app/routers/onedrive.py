...
from app.schemas.onedrive_schema import (
    ProcessFileRequest, ProcessFileResponse, GetDataRequest, GetDataResponse,
    FileTrackerResponse, SheetListResponse, HeaderMappingResponse,
    TenderDataResponse, PowerBIDataResponse, StatisticsResponse, SheetDataResponse
)
from app.models.onedrive import ExcelFileTracker, RawExcelSheet, SheetHeaderMapping, FileStatus
...
async def process_onedrive_file(
    ...
    share_link = request.share_link or settings.ONEDRIVE_SHARE_LINK
    if not share_link:
        raise HTTPException(status_code=400, detail="No OneDrive share link provided")

    processor = ExcelProcessor(db)

    if request.force_refresh:
        background_tasks.add_task(processor.process_onedrive_file, share_link, request.force_refresh)
        return ProcessFileResponse(
            success=True,
            message="File processing started in background",
            file_tracker=None
    ...
    file_tracker = await processor.process_onedrive_file(share_link, request.force_refresh)
    if not file_tracker:
        raise HTTPException(status_code=500, detail="Failed to process OneDrive file")
...
async def list_files(
    ...
    if status:
        try:
            status_enum = FileStatus(status.upper())
            stmt = stmt.where(ExcelFileTracker.status == status_enum)
        except ValueError:
            raise HTTPException(status_code=400, detail=f"Invalid status: {status}")
...
async def get_file_info(file_id: int, db: AsyncSession = Depends(get_db)):
    stmt = select(ExcelFileTracker).where(ExcelFileTracker.file_id == file_id)
...
# ------------------ List sheets ------------------ #
@router.get("/files/{file_id}/sheets", response_model=SheetListResponse)
async def list_sheets(file_id: int, db: AsyncSession = Depends(get_db)):
    stmt = select(ExcelFileTracker).where(ExcelFileTracker.file_id == file_id)
    result = await db.execute(stmt)
    file = result.scalars().first()
    if not file:
    ...
    stmt_sheets = select(RawExcelSheet.sheet_name).where(RawExcelSheet.file_id == file_id).distinct()
    result_sheets = await db.execute(stmt_sheets)
    sheets = [row[0] for row in result_sheets.all()]

    return SheetListResponse(file_id=file_id, sheets=sheets)

# ------------------ Get sheet data ------------------ #
@router.post("/data", response_model=GetDataResponse)
async def get_sheet_data(request: GetDataRequest, db: AsyncSession = Depends(get_db)):
    processor = ExcelProcessor(db)
    data = await processor.get_sheet_data(
        file_id=request.file_id,
        sheet_name=request.sheet_name,
        limit=request.limit,
        offset=request.offset
    ...
    # Total count
    stmt_count = select(func.count(RawExcelSheet.raw_id))
    if request.file_id:
        stmt_count = stmt_count.where(RawExcelSheet.file_id == request.file_id)
    if request.sheet_name:
        stmt_count = stmt_count.where(RawExcelSheet.sheet_name == request.sheet_name)
    ...
    file_info = None
    if request.file_id:
        stmt_file = select(ExcelFileTracker).where(ExcelFileTracker.file_id == request.file_id)
        result_file = await db.execute(stmt_file)
        file = result_file.scalars().first()
        if file:
            file_info = FileTrackerResponse.from_orm(file)
...
# ------------------ Sheet mapping ------------------ #
@router.get("/sheets/{sheet_name}/mapping", response_model=HeaderMappingResponse)
async def get_sheet_mapping(sheet_name: str, db: AsyncSession = Depends(get_db)):
...
async def get_normalized_tenders(
    ...
    if source_website:
        stmt = stmt.where(RawExcelSheet.source_website == source_website)
    if sheet_name:
        stmt = stmt.where(RawExcelSheet.sheet_name == sheet_name)
    if tender_id:
        stmt = stmt.where(RawExcelSheet.tender_id == tender_id)

    stmt = stmt.order_by(RawExcelSheet.created_at.desc()).offset(offset).limit(limit)
    result = await db.execute(stmt)
    rows = result.scalars().all()

    tenders = []
    for row in rows:
        tenders.append(TenderDataResponse(
            tender_id=row.row_json.get('normalized_tender_id'),
            title=row.row_json.get('normalized_title'),
            organization=row.row_json.get('normalized_organization'),
            deadline=row.row_json.get('normalized_deadline'),
...
async def get_powerbi_data(
    ...
    if date_from:
        try:
            ...
            stmt = stmt.where(RawExcelSheet.created_at >= dt_from)
        except ValueError:
            raise HTTPException(status_code=400, detail="Invalid date_from format")
    if date_to:
        try:
            dt_to = datetime.fromisoformat(date_to)
            stmt = stmt.where(RawExcelSheet.created_at <= dt_to)
        except ValueError:
            raise HTTPException(status_code=400, detail="Invalid date_to format")
    ...
    # latest file
    stmt_latest = select(ExcelFileTracker).order_by(ExcelFileTracker.uploaded_at.desc())
    result_latest = await db.execute(stmt_latest)
    latest_file = result_latest.scalars().first()

    return PowerBIDataResponse(
        data=data,
        metadata={
            'source_websites': list(set([r.source_website for r in rows if r.source_website])),
            'sheets': list(set([r.sheet_name for r in rows])),
            'date_range': {
                'from': min([r.created_at for r in rows]).isoformat() if rows else None,
                'to': max([r.created_at for r in rows]).isoformat() if rows else None
            }
        },
        last_updated=latest_file.uploaded_at if latest_file else datetime.utcnow(),
        total_records=len(data)
...
async def get_statistics(db: AsyncSession = Depends(get_db)):
    # Total files
    result_total = await db.execute(select(func.count(ExcelFileTracker.file_id)))
    total_files = result_total.scalar_one()

    # Files by status
    result_status = await db.execute(
        select(ExcelFileTracker.status, func.count(ExcelFileTracker.file_id))
        .group_by(ExcelFileTracker.status)
    )
    files_by_status = {status.value: count for status, count in result_status.all()}

    # Total rows
    result_rows = await db.execute(select(func.count(RawExcelSheet.raw_id)))
    total_rows = result_rows.scalar_one()

    # Total unique sheets
    result_sheets = await db.execute(select(func.count(func.distinct(RawExcelSheet.sheet_name))))
    total_sheets = result_sheets.scalar_one()

    # Rows by source
    result_source = await db.execute(
        select(RawExcelSheet.source_website, func.count(RawExcelSheet.raw_id))
        .group_by(RawExcelSheet.source_website)
    )
    rows_by_source = {source or 'Unknown': count for source, count in result_source.all()}

    # Latest file
    result_latest = await db.execute(select(ExcelFileTracker).order_by(ExcelFileTracker.uploaded_at.desc()))
    latest_file = result_latest.scalars().first()
