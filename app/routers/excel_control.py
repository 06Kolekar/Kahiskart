...
# ===============================
# UTILITY: Download Excel (Server Safe)
# ===============================

async def download_excel_if_url(path: str) -> str:
    """
    If path is a URL, download it to UPLOAD_DIR.
    Otherwise return original path.
    """

    if path.startswith("http://") or path.startswith("https://"):
        filename = f"excel_{datetime.utcnow().timestamp()}.xlsx"
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            if response.status_code != 200:
                raise HTTPException(
                    status_code=400,
                    detail=f"Failed to download Excel file. Status: {response.status_code}"
                )
...
async def process_excel_file_background(file_id: int):
    async with AsyncSessionLocal() as db:
        try:
            print(f"🔄 Background import started for file ID: {file_id}")
    ...
    try:
        ...
        for row in rows:
            ...
            source_name = (
                data.get('Source')
                or data.get('source')
                or data.get('City/Agency')
            )

            source_url = (
                data.get('Web Source Data Link (Links of Tender Release Sources)')
                or data.get('Column3 (Source Link 1)')
                or data.get('Column4')
            )
            ...
            login_required = (
                data.get('User Login Required?') == 'Yes'
                or data.get('User') not in [None, '', '-']
            )
            ...
            new_source = Source(
                ...
                status='ACTIVE',
                is_active=True,
                description=f"Auto-imported from Excel",
                created_at=datetime.utcnow()
            )
...
async def register_excel_file(
    ...
    # 🔥 If it's URL, download it first
    path = await download_excel_if_url(path)
    ...
    async with AsyncSessionLocal() as db:
        ...
        return {
            "status": "registered",
            "id": excel_file.id,
            "auto_import_triggered": auto_import
        }
...
async def get_import_status(db: AsyncSession = Depends(get_db)):
    ...
    tender_count = await db.scalar(
        ...
        .where(Tender.imported_from_excel == True)
    )

    source_count = await db.scalar(
        select(func.count()).select_from(Source)
        .where(Source.scraper_type == 'excel')
    )
    ...
    return {
        ...
        "imported_tenders": tender_count,
        "excel_sources": source_count,
        "import_rate": f"{(tender_count / excel_count * 100):.1f}%"
        if excel_count else "0%"
    }
